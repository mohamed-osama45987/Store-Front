...
describe('Order handler tests', () => {
  it('/api/orders should return orders of the user if given an id', async () => {
    const response = await request.get('/api/orders/user/1').set('Authorization', token)

    expect(response.status).toBe(200)
    expect(response.body.userOrders.length).toBeGreaterThan(0)
  })

  it('/api/orders/user/:id should create an order', async () => {
    const response = await request.post('/api/orders/user/1').set('Authorization', token).send({
      quantity: 5,
      userId: 1,
      productId: 1
    })
